...
export default function Cart(props) {
  ...
  const [singlePorductsCart, setSingleProductsCart] = useState([]);
  ...
  const decreaseQuantity = (id) => {
    const arrayItemsCart = productsCart;
    const result = removeIndexArray(arrayItemsCart, id.toString());
  ...
  return (
    <>
      <Button className="cart" variant="link">
      ...
      <div className="cart-content" style={{ width: widthCartContent }}>
        ...
        <div className="cart_content__products">
          {singlePorductsCart.map((idProductCart, index) => (
            <CartContentProducts
              key={index}
              products={products}
